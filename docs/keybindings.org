...
press =: helpful-at-point= (or =M-x helpful-at-point=) from [[https://github.com/Wilfred/helpful][helpful]] package to see
the full documentation about it. Bind it to key for more convenient access. Vim
uses =K= key for such things (which is available while there is only one cursor in
the buffer), another good choice is =M= since it's unused in Hel.
...
In Motion state Hel provide service keys to control Emacs itself.

- =:= :: Emacs analoug of command pallete. (Native key is =M-x=)

- =C-:= ::
  Open command palette with only the commands that make sense in the current
  buffer. (Native key is =M-X=)

- =M-u= ::
  Emacs universal argument. Native key is =C-u=, but Hel use it for scrolling,
  and =M-u= by default is bound to ~upcase-word~, for which Hel has itself version
  on =M-`= or =gU=.

- =SPC= :: Leader key (if ~hel-leader~ extension is installed)
...
- =C-w c= :: Close current window, if it was the last window on the tab —
  close the tab.
...
- =C-w= + =h=, =j=, =k=, =l= :: Go to the window toward the direction.

- =C-w= + =H=, =J=, =K=, =L= :: Move window in direction.

- =C-w RET= :: Display the buffer of the next command in the same window.

- =C-w n= :: Display the buffer of the next command in a new window.

- =C-w r= :: Revert buffer.

All window commands also duplicated with =C-= prefix for convenience: =C-w C-s=,
=C-w C-v=, etc.

A secondary prefix for window commands is available on =C-c w=. It is useful when
the =C-w= prefix is unavailable, such as in terminal emulators, ~picture-mode~,
and similar modes.
...
All keys available in Motion state are also work in Normal state.
...
- =gh= / =gl= :: Move to the *beginning* / *end* of a line.

- =gs= :: Move to the beginning of a line skipping indentation.

- =gg= ::
  Move to the beginning of the buffer. With numeric argument go to N/10 of the
  way from the beginning.

- =ge= or =G= ::
  Move to the end of the buffer. With a numeric argument, go to line N.

- ={= / =}= or =[p= / =]p= :: Move over paragraphs.

- =(= / =)= ::
  When there is only one cursor in the buffer, moves over sentences in major
  modes derived from ~text-mode~, or over functions in major modes derived from
  ~prog-mode~.

- =[f= / =]f= :: Move over functions.

- =[s= / =]s= :: Move over sentences.
...
- =d= :: Cut selection: delete it from buffer and put into ~kill-ring~.
...
- =y= ::
  - Copy the selection into ~kill-ring~.
  - With =M-u= (~universal-argument~) append selection to the last item in the
    ~kill-ring~.
...
- =`= / =M-`= or =gu= / =gU= ::
  Convert selection to *lower* / *upper* case. With no selection convert count
  characters after cursor. (Useful, for example, when you want to capitalize
  only the first letter of a word — place cursor before word and press =gU=
  with no selection.)
...
- =x= / =X= ::
  ...
  invocation. On subsequent calls *extend* / *contract* linewise selection down
...
- =ms= :: Surround selection.
  ...
  Left brackets — =(=, =[=, ={= — add extra spaces around the surrounded region.
  If the selection is linewise (make with =x=), the delimiters are placed on
  separate lines.
  
  Right brackets — =)=, =]=, =}= — do the opposite: they remove all spaces or
  newlines from the selection and are inserted tightly.
...
- =[x= / =]x= ::
  ...
  to have these positions in a stack separate from the jumplist.
